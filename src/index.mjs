...
import { filter, from, tap, timer } from 'rxjs';
...
const fetchDepartureData = () => {
  ...
  return from(axios.get(DEPARTURE_URL, options)).pipe(map(response => response.data));
};
...
console.log(`Starting script: interval ${intervalMs} ms`);
...
function getRandomProverb() {
  try {
    return proverbs[Math.floor(Math.random() * proverbs.length)].proverb;
  } catch (error) {
...
timer(0, intervalMs)
  .pipe(
    ...
    map(data => mapDepartureData(data)),
    filter(queryString => queryString.length > 0),
    map(queryString => {
      let proverb = wrap(getRandomProverb());
      return `${OUTPUT_URL_BASE}&${queryString}&fetchedTimestamp=${encodeURIComponent(formatDate(new Date()))}&foo=${proverb}`;
    }),
    switchMap(finalUrl => from(axios.post(finalUrl)).pipe(map(response => response.data)))
  )
  .subscribe({
